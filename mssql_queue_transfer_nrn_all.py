...
SELECT
    ...
    c.REMARK, cs.UNITPRICE, cs.NETAMOUNT, d8.code AS dim5_code, d8.LName AS dim5_name
FROM cssale c
...
LEFT JOIN CSDIM8 d8 ON d8.ID = cd.DIM6
...
SELECT
    ...
    cw.code, cw.lname, cl.code AS location_code, cl.lname AS location,
    cl.SNAME, cun.LName AS LName_unit, CAST(cs.QTY AS DECIMAL(8,2)) AS qty,
    c.REMARK, '0' AS UNITPRICE, '0' AS NETAMOUNT, d8.code AS dim5_code, d8.LName AS dim5_name
FROM CSSTKTF c
...
SELECT
    ...
    l.REMARK, le.UNITPRICE, le.NETAMOUNT, d8.code AS dim5_code, d8.LName AS dim5_name
FROM CSSALERET l
...
SELECT
    ...
    cu.lname AS user_lname, dg.lname AS dg_lname, cd.code AS cd_code,
    cs.id AS sub_id, cd.name AS cd_name, ct.lname AS ct_lname, cb.LName AS brand,
    cw.code, cw.lname, d8.code AS location_code, d8.LName AS location,
    cl.SNAME, cun.LName AS LName_unit, CAST(cs.QTY AS DECIMAL(8,2)) AS qty,
    c.REMARK, cs.UNITPRICE, cs.NETAMOUNT, d8.code AS dim5_code, d8.LName AS dim5_name
FROM CSPO c
...
LEFT JOIN CSUNIT cun ON cun.id = cs.UNITID
LEFT JOIN CSBRANCH ch ON ch.ID = c.BRANCH
LEFT JOIN CSDIM8 d8 ON d8.ID = cd.DIM7
INNER JOIN DateAndWarehouseFilter dwf ON c.DOCDATE >= dwf.TargetStartDate
WHERE cd.DIM6 <> '-1' AND c.VENDORNAME = '{vendor_name}'
...
TRANSFER_JOBS = {
    ...
    "nk": TransferJob(
        "nk",
        "NKVAT",
        PO_QUERY_TEMPLATE.format(vendor_name="บริษัท ยะลานำรุ่ง จำกัด (สาขา 2)"),
    ),
    "nrt": TransferJob(
        "nrt",
        "NRTVAT",
